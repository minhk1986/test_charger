...
void modbus::Getparam(ros::NodeHandlePtr nodeHandle, fx5ucParam &param)
{
    char paramName[70];
	sprintf(paramName, "/IP");
	nodeHandle->getParam(paramName, param.ip_address);
	sprintf(paramName, "/PORT");
	nodeHandle->getParam(paramName, param.port);
	sprintf(paramName, "/ID");
	nodeHandle->getParam(paramName, param.id);
}
...
modbus::modbus(ros::NodeHandlePtr nodeHandle) 
{
    ...
    HOST = this->plcParam.ip_address;
    PORT = (uint16_t)this->plcParam.port;
    _slaveid = this->plcParam.id;
    _msg_id = 1;
    _connected = false;
    err = false;
    err_no = 0;
