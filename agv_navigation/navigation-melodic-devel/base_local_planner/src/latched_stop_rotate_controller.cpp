...
bool LatchedStopRotateController::stopWithAccLimits(const geometry_msgs::PoseStamped& global_pose,
  ...
  ROS_INFO("latched_stop_rotate_controller.cpp-120-stopWithAccLimits()");
  ...
  if(valid_cmd){
    ROS_INFO_ONCE("latched_stop_rotate_controller.cpp-139-Slowing down... using vx, vy, vth: %.2f, %.2f, %.2f", vx, vy, vth);
    cmd_vel.linear.x = vx;
    cmd_vel.linear.y = vy;
